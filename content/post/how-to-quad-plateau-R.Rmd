...
```{r qp-2}
quad_plateau <- function(data, confidence = 90, plot = TRUE) {
    # initial values
    ini_fit <- lm(yield ~ potassium, data = data)
    ...
    # get model coefficients back for plotting
    mod_a  <- coef(corr_model)[[1]]
    mod_b  <- coef(corr_model)[[2]]
    mod_c <- coef(corr_model)[[3]]
    curve_qp <- -0.5 * mod_b / mod_c
    plateau_qp <-
        mod_a + mod_b * mod_c + (-0.5 * mod_b / mod_c) * mod_c * mod_c
    
    # Printout
    print("==========Pseudo-R-squared==========")
    print(pseudo_r2$Pseudo.R.squared.for.model.vs.null)
    print("==========Model Summary==========")
    print(summary(corr_model))
    print("==========Equation==========")
    print(paste(round(mod_a, 2),"+",
                round(mod_b, 2),
                "x",
                "+",
                round(mod_c, 2),
    ...
    print(ci)
    print(paste("Quad coefficient", round(curve_qp, 4)))
    print(paste("Quadratic-plateau at ", round(plateau_qp, 1), "% RY"))

    # check residuals
    par(mfrow = c(2, 2))
    plotNormalHistogram(residuals(corr_model))
    plot(fitted(corr_model), residuals(corr_model))
    par(mfrow = c(1, 1))
    ...
    if (plot == TRUE)
      ...
      geom_point(size = 1, alpha = 0.5) +
      geom_rug(alpha = 0.5, length = unit(2, "pt")) +
      geom_vline(xintercept = mod_c,
                 color = "#CC0000") +
      geom_vline(
        xintercept = c(cil, cih),
        color = "#CC0000",
        alpha = 0.2
      ) +
      geom_smooth(
      ...
      scale_y_continuous(breaks = seq(0, 200, 5)) +
      labs(
        caption = paste("Each point is a site-year. n =", nrow(data)),
        subtitle = paste0(
          "Plateau =",
          round(plateau_qp, 1),
          "% RY.  ",
